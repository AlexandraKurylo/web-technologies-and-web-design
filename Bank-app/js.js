...
const account1 = {
  ...
  movementsDates: [
    ...
    "2025-09-10T17:01:17.194Z",
    "2025-09-14T23:36:17.929Z",
    "2025-09-15T10:51:36.790Z",
  ],
  currency: "UAH",
  locale: "pt-PT",
};
...
function formatMovementDate(date) {
  ...
  if (daysPassed >= 2 && daysPassed <= 4) return `Пройшло ${daysPassed} дні`;
...
// Виведення на сторінку всіх надходжень та відтоків
function displayMovements(acc, sort = false) {
  ...
  movs.forEach(function (value, i) {
    ...
    const typeMessage = value > 0 ? "внесення" : "зняття";
    // Перебір масиву з датами
...
// Підрахунок та виведення на сторінку загального балансу
...
// Сума та виведення на сторінку надходження та відтоку у footer
...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount && currentAccount.pin === Number(inputLoginPin.value)) {
    ...
    // Оновлення поточної дати після входу до облікового запису
...
// Переказ грошей на інший акаунт
btnTransfer.addEventListener("click", function (e) {
  ...
  if (
    ...
    // Додавання дати в mov
    currentAccount.movementsDates.push(new Date().toISOString());

    //Оновлення таймера під час переказу
    clearInterval(timer);
    timer = startLogOut();
    updateUi(currentAccount);
...
btnLoan.addEventListener("click", function (e) {
  ...
  if (amount > 0) {
    ...
    // Додавання дати в масив movementsDates
    currentAccount.movementsDates.push(new Date().toISOString());

    // Оновлення таймера під час внесення коштів
    clearInterval(timer);
    timer = startLogOut();
...
// Сортування по надходженнях та відтоках
let sorted = false;
